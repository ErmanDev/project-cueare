...
async function seed() {
  ...
  try {
    ...
    let userRes = await client.query(
      'SELECT "userId" FROM ssc."Users" ORDER BY "userId" ASC LIMIT 1',
    );
    let userId: number;
    ...
    const templates = [
      {
        ...
        description:
          "Standard attendance fine rules for regular campus events and academic seminars.",
        maxFine: 500.0,
        rules: [
          {
            sessionType: "GENERAL",
      ...
      },
      {
        ...
        description:
          "Enforced for university-wide mandatory events, foundation days, and general convocations.",
        maxFine: 1000.0,
        rules: [
          {
            sessionType: "GENERAL",
      ...
      },
      {
        ...
        description:
          "Lightweight fine policy for student council meetings and general assemblies.",
        maxFine: 300.0,
        rules: [
          {
            sessionType: "GENERAL",
      ...
      },
      {
        ...
        description:
          "Standard multi-session policy with differentiated AM and PM absence and tardiness rates.",
        maxFine: 500.0,
        rules: [
          {
            sessionType: "AM",
    ...
    for (const t of templates) {
      ...
      console.log(
        `✓ Seeded template "${t.name}" (${t.code}) with Version 1 and ${t.rules.length} rules.`,
      );
    }
